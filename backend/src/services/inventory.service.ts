...
export class InventoryService {
   ...
   */
  static async getStockSummary(itemId?: string) {
    const records = await prisma.inventory.findMany({
      where: itemId ? { itemId } : {},
   ...
   */
  static async stockIn(dto: StockInDto) {
    if (dto.quantity <= 0) {
   ...
   */
  static async adjustStock(dto: AdjustStockDto) {
    if (dto.newPhysicalQuantity < 0) {
      throw new BadRequestError('Physical quantity cannot be negative');
    }
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (!existing) {
        throw new NotFoundError('Inventory batch record not found to adjust');
      }

      if (dto.newPhysicalQuantity < existing.reservedQuantity) {
        throw new BadRequestError(
          `Cannot adjust physical quantity to ${dto.newPhysicalQuantity} because ${existing.reservedQuantity} units are currently reserved`
        );
      }
